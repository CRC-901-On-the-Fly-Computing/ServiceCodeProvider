...
int32_t run_service_scaling_gpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source, int32_t rows, int32_t columns, int32_t scale_up_factor, int32_t scale_down_factor) {
	...
	if(rows%scale_down_factor != 0) { rows_down = rows_up/scale_down_factor+1; }
	else rows_down = (rows_up)/scale_down_factor;
	if(columns%scale_down_factor != 0) { columns_down = columns_up/scale_down_factor+1; }
	...
	#pragma acc kernels copyin(pixpack_source[0:columns*rows]) copyout(pixpack_target[0:columns_down*rows_down]) create(pixpack_temp[0:columns_up*rows_up])
	{
		#pragma acc loop independent private(k)
		for (i = 0; i < rows_up; i++) {
			k = i / scale_up_factor;
			#pragma acc loop independent private(l, pos, pos_temp, temp_red, temp_green, temp_blue)
	...
	//{
		...
		for (i = 0; i < rows_up - 1; i += scale_down_factor) {
			...
			#pragma acc loop independent private(l, pos_temp, pos_target, temp_red, temp_green, temp_blue)
			for (j = 0; j < columns_up - 1; j += scale_down_factor) {
				...
				if (scale_down_factor != 1) {
					...
					for (n = 0; n < (scale_down_factor * scale_down_factor) / 2; n++) {
						if((pos_temp + n) < columns_up * rows_up ){
					...
					for (n = 0; n < (scale_down_factor * scale_down_factor) / 2; n++) {
						if((pos_temp + columns_up+ n) < columns_up * rows_up ){
