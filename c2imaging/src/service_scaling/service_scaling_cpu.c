...
int32_t run_service_scaling_cpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source, int32_t rows, int32_t columns, int32_t scale_up_factor, int32_t scale_down_factor) {
	...
	#pragma omp parallel
	{
		#pragma omp for private(i, j, k, l, pos, pos_temp,temp_red, temp_green, temp_blue)
		for (i = 0; i < rows_up; i++) {
			k = i / scale_up_factor;
	...
	//#pragma omp parallel
	//{
		#pragma omp for private(i, j, k, l, pos_temp, pos_target,temp_red, temp_green, temp_blue)
