...
typedef int32_t (*run_service_gaussblur_gpu_t)(PixelPacket *, PixelPacket *, int32_t, int32_t, int32_t);
...
void run_service(char resource, void *handler_gpu, Image **target_images, int32_t *target_images_size, Image *source_images, int32_t source_images_size, double *params, int32_t *status_code) {
	...
	*target_images_size			= 1;
	...
	run_service_gaussblur_gpu_t run_service_gaussblur_gpu;
	char *dlsym_error;
	
	switch (resource) {
		case 'c':
			*status_code = run_service_gaussblur_cpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (int32_t)params[0]);
			break;
		case 'g':
			#ifdef PGCC
			handler_gpu = dlopen("../bin/plugins/libservice_gaussblur_gpu.so", RTLD_LAZY | RTLD_GLOBAL);
			if (NULL == handler_gpu) {
				fprintf(stderr, "!!! ERROR: Cannot open library libservice_gaussblur_gpu.so: %s (file: %s, line: %i)\n", dlerror(), __FILE__, __LINE__);
				exit(EXIT_FAILURE);
			}
			#endif
			run_service_gaussblur_gpu = (run_service_gaussblur_gpu_t) dlsym(handler_gpu, "run_service_gaussblur_gpu");
			dlsym_error = dlerror();
			if (dlsym_error) {
				fprintf(stderr, "!!! ERROR: Cannot load symbol \"run_service_gaussblur_gpu\": %s (file: %s, line: %i)\n", dlsym_error, __FILE__, __LINE__);
				break;
			}
			*status_code = run_service_gaussblur_gpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (int32_t)params[0]);
			#ifdef PGCC
			dlclose(handler_gpu);
			#endif
			break;
		case 'f':
			fprintf(stderr, "!!! ERROR: Gaussblur does not have an FPGA implementation.\n");
			*status_code = -1;
			break;
		case 's':
		default:
			*status_code = run_service_gaussblur_scpu((*target_images)[0].image, source_images[0].image, source_images[0].rows, source_images[0].columns, (int32_t)params[0]);
			break;
	}
}
...
void get_service_info(char ***info_strings, int32_t *info_strings_size) {
	char *strings[] = {
		"Blur an image with Gaussian blur filter."
	};
	*info_strings_size = 1;
...
void get_service_params(char ***param_strings, int32_t *param_strings_size) {
	char *strings[] = {
		"Number of rows/columns of the filter matrix."
	};
