...
int32_t run_service_resize_gpu(PixelPacket *pixpack_target, PixelPacket *pixpack_source, int32_t rows, int32_t columns, int32_t resize_factor) {
	...
	#pragma acc kernels copyin(pixpack_source[0:columns*rows]) copyout(pixpack_target[0:new_columns*new_rows])
	{
		#pragma acc loop independent private(k)
		for (i=0; i<rows-1; i+=2) {
			k = i>>1;
			#pragma acc loop independent private(pos, pos_target, l, temp_red, temp_green, temp_blue)
